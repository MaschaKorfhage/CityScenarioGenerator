...
def check_building_ids(buildings):
    """
    Checks if there are building IDs that only differ in case. If so, and if
    running on Windows (which uses case-insensitive filenames), the IDs are
    adapted to make them unique without case.
